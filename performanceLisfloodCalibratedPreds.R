...
plot(Run_hetGP_mean, main= "Mean prediction from HetGP10 emulation-calibration", breaks=cuts, col = pal(length(cuts)+1))
...
Run_hetML_diff<- RunTrue.10m
values(Run_hetML_diff)<- hetML_mean - truevals.10m

Run_homML_diff<- RunTrue.10m
values(Run_homML_diff)<- homML_mean - truevals.10m

Run_hetGP_diff<- RunTrue.10m
values(Run_hetGP_diff)<- hetGP_mean - truevals.10m
...
#plot(RunTrue.10m)
par(mfrow=c(2,2))
plot(Run_hetML_diff, main= "Mean prediction from HetML emulation-calibration minus observation")
plot(Run_homML_diff, main= "Mean prediction from HomML emulation-calibration minus observation")
plot(Run_hetGP_diff, main= "Mean prediction from HetGP emulation-calibration minus observation")
plot(Run_homGP_diff, main= "Mean prediction from HomGP emulation-calibration minus observation")


Run_hetML_sd<- RunTrue.10m
values(Run_hetML_sd)<- hetML_sd

Run_homML_sd<- RunTrue.10m
values(Run_homML_sd)<- homML_sd

Run_hetGP_sd<- RunTrue.10m
values(Run_hetGP_sd)<- hetGP_sd

Run_homGP_sd<- RunTrue.10m
values(Run_homGP_sd)<- homGP_sd

par(mfrow=c(2,2))
plot(Run_hetML_sd, main= "HetML sd")
plot(Run_homML_sd, main= "HomML sd")
plot(Run_hetGP_sd, main= "HetGP sd")
...
hetML_ED<- sqrt(sum((hetML_mean - truevals.10m)^2))
homML_ED<- sqrt(sum((homML_mean - truevals.10m)^2))
hetGP_ED<- sqrt(sum((hetGP_mean - truevals.10m)^2))
homGP_ED<- sqrt(sum((homGP_mean - truevals.10m)^2))

hetML_ED; homML_ED; hetGP_ED; homGP_ED
#RMSE
hetML_RMSE<- sqrt(mean((hetML_mean - truevals.10m)^2))
homML_RMSE<- sqrt(mean((homML_mean - truevals.10m)^2))
hetGP_RMSE<- sqrt(mean((hetGP_mean - truevals.10m)^2))
homGP_RMSE<- sqrt(mean((homGP_mean - truevals.10m)^2))

hetML_RMSE; homML_RMSE; hetGP_RMSE; homGP_RMSE

#PBIAS

hetML_PBias<- 100 * (sum( hetML_mean - truevals.10m ) / sum( truevals.10m ))  
homML_PBias<- 100 * (sum( homML_mean - truevals.10m ) / sum( truevals.10m ))  
hetGP_PBias<- 100 * (sum( hetGP_mean - truevals.10m ) / sum( truevals.10m ))  
...
Am<- length(which(hetML_mean>0))*mGridArea
Arm<- length(intersect(which(truevals.10m>0),which(hetML_mean>0)))*rGridArea
Fvals_hetML<- Arm/(Am + Ar - Arm)
Cvals_hetML<- Arm/Ar
...
Am<- length(which(hetGP_mean>0))*mGridArea
Arm<- length(intersect(which(truevals.10m>0),which(hetGP_mean>0)))*rGridArea
Fvals_hetGP<- Arm/(Am + Ar - Arm)
Cvals_hetGP<- Arm/Ar
...
save(hetML_ED, homML_ED, hetGP_ED, homGP_ED,
     Fvals_hetML, Fvals_homML, Fvals_hetGP, Fvals_homGP, 
     Cvals_hetML, Cvals_homML, Cvals_hetGP, Cvals_homGP,
     file="C:/FloodingModelCalibrationProject/multires/outputData/4Pars/prior1/10mfrom50m/calibration/LISFLOOD/ED_F_Cvals.RData")
...
plot(RunTrue.10m, main= "Observation")
par(mfrow=c(1,2))
plot(Run_hetGPCheap_mean, main= "Mean prediction from HetGP (Cheap) emulation-calibration")
plot(Run_homGPCheap_mean, main= "Mean prediction from HomGP (Cheap) emulation-calibration")

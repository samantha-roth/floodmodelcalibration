...
transformed data{
  ...
  matrix[n_e,n_e] sigma_var_trend;
  ...
  mu_v = v_H * v_beta_m;
	...
	sigma_var_trend =  (v_H*(v_beta_s))*(v_H') ; // var(Hbeta) (log noise)
	//corresponds to var_mean_lambda in hetGP code
...
Var_Y_e = (rho * rho * Var_Y_c[(1+n_c-n_e):n_c,(1+n_c-n_e):n_c] ) + exp_kern(D_ee, square(m_sigma), exp(-2*log(m_theta)), n_e, n_e, K) + diag_matrix(exp(logLambda)); 
...
var_var_cholesky = cholesky_decompose(v_var + sigma_var_trend);
// C_v(x_e,x_e) + diag(v_nugget) + sigma_var_trend
// corresponds to v_var in hetGP.stan

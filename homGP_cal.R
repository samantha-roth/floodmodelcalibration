#Calibration- HomGP with no delta estimate
...
mh.alg<- function(init, n.sample) {
  ...
  for (i in 1 : n.sample) {
    ...
    if(prop<.02 | prop>.1){
      ...
      prop.x.test<- matrix(c(prop, x2.out[i], x3.out[i], x4.out[i]),
                           nrow=1,
                           ncol= length(c(x1.out[i], x2.out[i], x3.out[i], x4.out[i])))
      
      prop.x.v1 <- scale(prop.x.test, center = attr(x.std, "scaled:center"), 
                         scale = attr(x.std, "scaled:scale"))
      
      #calculated quantities after fitting homSML emulator
      prop.H.new<- cbind(1, prop.x.test)
      prop.crosscov<- gp.crosscov(prop.x.v1, x.std, 
                                  pars$m_sigma, pars$m_theta, 
                                  prop.H.new, H, var.beta )
    ...
    x.test<- matrix(c(x1.out[i+1], x2.out[i], x3.out[i], x4.out[i]),
                    nrow=1,ncol= length(c(x1.out[i+1], x2.out[i],x3.out[i], x4.out[i])))
    ...
    #calculated quantities after fitting homSML emulator
    H.new <- cbind(1, x.test)
    crosscov <- gp.crosscov(x.v1, x.std, 
                            pars$m_sigma, pars$m_theta, 
                            H.new, H, var.beta )
    ...
    if(prop<.02 | prop>.4){
      ...
      prop.x.test<- matrix(c(x1.out[i+1], prop, x3.out[i], x4.out[i]),
                           nrow=1,
                           ncol= length(c(x1.out[i], x2.out[i], x3.out[i], x4.out[i])))
    ...
    x.test<- matrix(c(x1.out[i+1], x2.out[i+1], x3.out[i], x4.out[i]),
                    nrow=1,
                    ncol= length(c(x1.out[i+1], x2.out[i+1], x3.out[i], x4.out[i])))
    ...
    if(prop<.95 | prop>1.05){
      ...
      prop.x.test<- matrix(c(x1.out[i+1],x2.out[i+1], prop, x4.out[i]),
                           nrow=1,
                           ncol= length(c(x1.out[i+1],x2.out[i+1], prop, x4.out[i])))
    ...
    x.test<- matrix(c(x1.out[i+1], x2.out[i+1], x3.out[i+1], x4.out[i]),
                    nrow=1,
                    ncol= length(c(x1.out[i+1], x2.out[i+1], x3.out[i+1], x4.out[i])))
    ...
    #  x4.out[i+1]<- x4.out[i]
    #} else{
    prop.x.test<- matrix(c(x1.out[i+1], x2.out[i+1], x3.out[i+1], prop),
                         nrow=1, 
                         ncol=length(c(x1.out[i+1],x2.out[i+1], x3.out[i+1], x4.out[i])))
    ...
    x.test<- matrix(c(x1.out[i+1], x2.out[i+1], x3.out[i+1], x4.out[i+1]),
                    nrow=1,
                    ncol= length(c(x1.out[i+1], x2.out[i+1], x3.out[i+1], x4.out[i+1])))
...
#Try different seed and long run time to see what happens
pt<-proc.time() # Start Time
set.seed(215)
res <- mh.alg(init = init_vals, n.sample = 3e5)
ptFinal<-proc.time()-pt ; ptFinal<-ptFinal[3] # End Time to be used in Effective Samples per Second Calculation

save(res,file="C:/FloodingModelCalibrationProject/multires/outputData/4Pars/prior1/10mfrom50m/calibration/mcmc_res_homGPWHY.RData")
save(ptFinal,file="C:/FloodingModelCalibrationProject/multires/outputData/4Pars/prior1/10mfrom50m/calibration/time_homGPWHY.RData")



#save(res,file="C:/FloodingModelCalibrationProject/multires/outputData/4Pars/prior1/10mfrom50m/calibration/mcmc_res_homGP.RData")
#save(ptFinal,file="C:/FloodingModelCalibrationProject/multires/outputData/4Pars/prior1/10mfrom50m/calibration/time_homGP.RData")

save(res,file="C:/FloodingModelCalibrationProject/multires/outputData/4Pars/prior1/10mfrom50m/calibration/mcmc_res_homGP2.RData")
save(ptFinal,file="C:/FloodingModelCalibrationProject/multires/outputData/4Pars/prior1/10mfrom50m/calibration/time_homGP2.RData")

#load("C:/FloodingModelCalibrationProject/multires/outputData/4Pars/prior1/10mfrom50m/calibration/mcmc_res_homGP.RData")
#res1<- res
#load("C:/FloodingModelCalibrationProject/multires/outputData/4Pars/prior1/10mfrom50m/calibration/mcmc_res_homGP2.RData")
#res2<- res
...
#nPars=5
#for(i in 1:nPars){
#  plot(density(res[,i]),main=paste("HomGP variable ",i,sep=""),col="black")

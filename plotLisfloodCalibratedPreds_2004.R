...
max(rtp.homML$RunTrue2004_1)
max(rtp.homGP$RunTrue2004_1)
max(rtp.homGPCheap$RunTrue2004_1)
...
#par(mfrow=c(3,2))
rtp$RunTrue2004_1<- rtp$RunTrue2004_1/100
jpeg(filename = "C:/FloodingModelCalibrationProject/multires/plots/4Pars/10mfrom50m/prior1/calibrationResults/Observation2004.jpeg",width = 800, height = 600)
selinsgrove_ggmap + 
  geom_polygon(data = rtp, 
               aes(x = long, y = lat, group = group, 
                   fill = rep(rtp$RunTrue2004_1, each= 5)), 
               #fill=layer),
               size = 0, 
               alpha = 0.7)  + 
...
beepr::beep()
################################################################################
rtp.homML$RunTrue2004_1<- rtp.homML$RunTrue2004_1/100
jpeg(filename = "C:/FloodingModelCalibrationProject/multires/plots/4Pars/10mfrom50m/prior1/calibrationResults/HomML_meanpred2004.jpeg",width = 800, height = 500)
selinsgrove_ggmap + 
  geom_polygon(data = rtp.homML, 
               aes(x = long, y = lat, group = group, 
                   fill = rep(rtp.homML$RunTrue2004_1, each= 5)), 
               #fill=layer),
               size = 0, 
               alpha = 0.7)  + 
  #scale_fill_gradientn("Flood Height (cm)", colors = rev(heat.colors(800)), limits= c(0,900)) +
...
################################################################################
rtp.homGPCheap$RunTrue2004_1<- rtp.homGPCheap$RunTrue2004_1/100
jpeg(filename = "C:/FloodingModelCalibrationProject/multires/plots/4Pars/10mfrom50m/prior1/calibrationResults/HomGP50_meanpred2004.jpeg",width = 800, height = 500)
selinsgrove_ggmap + 
  geom_polygon(data = rtp.homGPCheap, 
               aes(x = long, y = lat, group = group, 
                   fill = rep(rtp.homGPCheap$RunTrue2004_1, each= 5)), 
               #fill=layer),
               size = 0, 
               alpha = 0.7)  + 
  #scale_fill_gradientn("Flood Height (cm)", colors = rev(heat.colors(800)), limits= c(0,900)) +
...
beepr::beep()
################################################################################
rtp.homGP$RunTrue2004_1<- rtp.homGP$RunTrue2004_1/100
jpeg(filename = "C:/FloodingModelCalibrationProject/multires/plots/4Pars/10mfrom50m/prior1/calibrationResults/HomGP10_meanpred2004.jpeg",width = 800, height = 500)
selinsgrove_ggmap + 
  geom_polygon(data = rtp.homGP, 
               aes(x = long, y = lat, group = group, 
                   fill = rep(rtp.homGP$RunTrue2004_1, each= 5)), 
               #fill=layer),
               size = 0, 
               alpha = 0.7)  + 
  #scale_fill_gradientn("Flood Height (cm)", colors = rev(heat.colors(800)), limits= c(0,900)) +
...
rtp.homMLdiff$RunTrue2004_1<- rtp.homMLdiff$RunTrue2004_1/100
...
rtp.homGPCheapdiff$RunTrue2004_1<- rtp.homGPCheapdiff$RunTrue2004_1/100
################################################################################
#HomGP10
...
rtp.homGPdiff$RunTrue2004_1<- rtp.homGPdiff$RunTrue2004_1/100
################################################################################

max(rtp.homMLdiff$RunTrue2004_1)
max(rtp.homGPdiff$RunTrue2004_1)
max(rtp.homGPCheapdiff$RunTrue2004_1)

min(rtp.homMLdiff$RunTrue2004_1)
min(rtp.homGPdiff$RunTrue2004_1)
min(rtp.homGPCheapdiff$RunTrue2004_1)

#par(mfrow=c(3,2))

maxmax<- round(max(c(max(rtp.homMLdiff$RunTrue2004_1),max(rtp.homGPdiff$RunTrue2004_1),max(rtp.homGPCheapdiff$RunTrue2004_1))),1)
minmin<- round(min(c(min(rtp.homMLdiff$RunTrue2004_1),min(rtp.homGPdiff$RunTrue2004_1),min(rtp.homGPCheapdiff$RunTrue2004_1))),1)
################################################################################
jpeg(filename = "C:/FloodingModelCalibrationProject/multires/plots/4Pars/10mfrom50m/prior1/calibrationResults/HomML_diffpred2004.jpeg",width = 800, height = 600)
selinsgrove_ggmap + 
  geom_polygon(data = rtp.homMLdiff, 
               aes(x = long, y = lat, group = group, 
                   fill = rep(rtp.homMLdiff$RunTrue2004_1, each= 5)), 
               #fill=layer),
               size = 0, 
               alpha = 0.7)  + 
  #scale_fill_gradientn("Flood Height (cm)", colors = rev(heat.colors(800)), limits= c(0,900)) +
...
################################################################################
jpeg(filename = "C:/FloodingModelCalibrationProject/multires/plots/4Pars/10mfrom50m/prior1/calibrationResults/HomGP50_diffpred2004.jpeg",width = 800, height = 600)
selinsgrove_ggmap + 
  geom_polygon(data = rtp.homGPCheapdiff, 
               aes(x = long, y = lat, group = group, 
                   fill = rep(rtp.homGPCheapdiff$RunTrue2004_1, each= 5)), 
               #fill=layer),
               size = 0, 
               alpha = 0.7)  + 
  #scale_fill_gradientn("Flood Height (cm)", colors = rev(heat.colors(800)), limits= c(0,900)) +
...
beepr::beep()
################################################################################
jpeg(filename = "C:/FloodingModelCalibrationProject/multires/plots/4Pars/10mfrom50m/prior1/calibrationResults/HomGP10_diffpred2004.jpeg",width = 800, height = 600)
selinsgrove_ggmap + 
  geom_polygon(data = rtp.homGPdiff, 
               aes(x = long, y = lat, group = group, 
                   fill = rep(rtp.homGPdiff$RunTrue2004_1, each= 5)), 
               #fill=layer),
               size = 0, 
               alpha = 0.7)  + 
  #scale_fill_gradientn("Flood Height (cm)", colors = rev(heat.colors(800)), limits= c(0,900)) +
...
#high residuals
################################################################################
jpeg(filename = "C:/FloodingModelCalibrationProject/multires/plots/4Pars/10mfrom50m/prior1/calibrationResults/HomML_diffpred2004g1.25.jpeg",width = 1000, height = 800)
selinsgrove_ggmap + 
  geom_polygon(data = rtp.homMLdiff, 
               aes(x = long, y = lat, group = group, 
                   fill = rep(rtp.homMLdiff$RunTrue2004_1, each= 5)), 
               ...
               alpha = 0.7)  + 
  #scale_fill_gradientn("Flood Height (cm)", colors = rev(heat.colors(800)), limits= c(0,900)) +
  scale_fill_gradientn("Difference (m)", colors = terrain.colors(100*(2*maxmax)), limits= c(1.25,maxmax)) +
  labs(title= "Prediction from HomMR emulation-calibration - Observation")
dev.off()
beepr::beep()

################################################################################
jpeg(filename = "C:/FloodingModelCalibrationProject/multires/plots/4Pars/10mfrom50m/prior1/calibrationResults/HomGP10_diffpred2004g1.25.jpeg",width = 1000, height = 800)
selinsgrove_ggmap + 
  geom_polygon(data = rtp.homGPdiff, 
               aes(x = long, y = lat, group = group, 
                   fill = rep(rtp.homGPdiff$RunTrue2004_1, each= 5)), 

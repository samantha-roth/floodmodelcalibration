...
residual_df<- data.frame(
  "Residuals"= c(homML_diff_urban, homGP_diff_urban, homGPCheap_diff_urban),
  "Source"= c(rep("HomMR",length(homML_diff_urban)),
              rep("HomGP10",length(homGP_diff_urban)),
              rep("HomGP50",length(homGPCheap_diff_urban)))
)


#plot(density(homML_diff_urban), col= "red", main= "PDF of residuals for 2011 flood- Urban areas", xlim= c(-.5,2.6))
#lines(density(homGP_diff_urban), col= "blue")
#lines(density(homGPCheap_diff_urban), col= "turquoise")
#legend(1,3 , legend=c("HomMR Residual", 
#                      "HomGP10 Residual",
#                      "HomGP50 Residual"),
#       col=c("red",
#             "blue", 
#             "turquoise"), lty=1,cex=1.2,box.lty = 0.1)

...
if(prior==1){
  if(extra==FALSE){
  	...
  	writeRaster(model_hazard,
    	paste0("./Outputs50m/prior1/Extent/Run_", i, ".asc"),
   	format = "ascii",
    	overwrite = T)
  
  	colnames(outputs) <-c("run no.","n_ch","n_fp","rwe","ree")
  	write.csv(outputs, paste0("./Outputs50m/prior1/Run_", i, ".csv"))
   ...
   if(extra==TRUE){
  	writeRaster(model_hazard,
    	paste0("./Outputs50m/prior1/extra/Extent/Run_", i, ".asc"),
    	format = "ascii",
    	overwrite = T)
  
  	colnames(outputs) <-c("run no.","n_ch","n_fp","rwe","ree")
  	write.csv(outputs, paste0("./Outputs50m/prior1/extra/Run_", i, ".csv"))
...
if(prior==2){
  ...
  writeRaster(
    model_hazard,
    paste0("./Outputs50m/prior2/Extent/Run_", i, ".asc"),
    format = "ascii",
    overwrite = T
  )
  ...
  write.csv(outputs, paste0("./Outputs50m/prior2/Run_", i, ".csv"))
}
